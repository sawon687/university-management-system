...
export const createPaymentValidationSchema = z.object({
	body: z
		.object({
			...
			feeId: z.string().trim().min(1, "Fee ID is required").optional(),

			semesterFees: z.coerce
				.number()
		...
		.superRefine((data, ctx) => {
			...
			if (data.applicationsId && data.feeId) {
				ctx.addIssue({
					code: "custom",
					message: "You cannot provide applicationsId and feeId together",
				});
			}
			...
			if (data.applicationsId && data.semesterFees !== undefined) {
				ctx.addIssue({
					code: "custom",
					path: ["semesterFees"],
					message: "semesterFees is not allowed for admission payment",
				});
			}
		}),
});
